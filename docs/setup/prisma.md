...
- `prisma.config.ts` na raiz define a URL via `DIRECT_URL` (Prisma 7 move a URL do datasource para fora do `schema.prisma`) — não `DATABASE_URL`, ver correção de produção abaixo.
...
- Ambiente local vs produção: `prisma.config.ts` e `prisma/seed.ts` carregam `.env.local` antes de `.env` (mesma precedência do Next.js), então basta configurar `DATABASE_URL`/`DIRECT_URL` no `.env.local` para apontar para um Postgres local (ex: `docker compose up -d`, ver `docker-compose.yml` na raiz, mesma URL para as duas variáveis já que não há pooler local) sem afetar as URLs de produção guardadas em `.env`.
- **Correção de produção (2026-09):** Prisma Postgres expõe duas connection strings do mesmo banco — uma *pooled* (host `pooled.db.prisma.io`, para tráfego de aplicação) e uma *direta* (host `db.prisma.io`, para migrations/CLI/admin, com cota de conexões concorrentes pequena). O projeto usava a mesma URL direta em `DATABASE_URL` (runtime, `src/lib/prisma.ts`) e em `prisma.config.ts` (`prisma migrate deploy`). Sob tráfego serverless concorrente, a aplicação esgotava a cota de conexões diretas, e o build da Vercel passou a falhar em `prisma migrate deploy` com `FATAL: too many connections for role "prisma_migration"`. Fix: `DATABASE_URL` agora é a URL *pooled* (usada só pela aplicação), e `DIRECT_URL` é a URL *direta* (usada só por `prisma.config.ts`, isolando o tráfego de app do orçamento de conexões que as migrations precisam).
