...
Fase de bootstrap concluída, e nove features já implementadas com dados reais (cada uma numa branch `feature/*` mergeada em `main` via PR):
...
- **Vagas de emprego** (`/vagas`, `/vagas/[id]`, `/painel/vagas`): CRUD de `Job` a partir do painel da empresa (Server Actions em `src/app/(business)/painel/vagas/actions.ts`, validação em `src/lib/validations/job.ts` — exige `contactEmail` ou `applicationUrl`, faixa salarial opcional com `showSalary`), listagem pública filtrável por cidade/tipo/modalidade e página de detalhe. Fechamento automático de vagas com `closesAt` vencido via cron diário (`vercel.json` → `GET /api/cron/close-expired-jobs`, protegido por `CRON_SECRET`, `03:00`). Ainda sem doc de arquitetura dedicada em `docs/` — nem o modelo `Job` está documentado em `docs/arquitetura/modelo-de-dados.md` hoje.
